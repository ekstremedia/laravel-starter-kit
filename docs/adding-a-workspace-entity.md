...
class Car extends Model implements FileOwner
{
    ...
    protected $fillable = ['make', 'model']; // workspace_id is stamped automatically
...
with `Car::withoutGlobalScope('workspace')`.
...
## 5. Controller — no workspace filtering needed
...
class CarController extends Controller
{
    ...
    {
        // workspace_id is stamped by the trait on create.
...
scoped to the active workspace by `ResolveWorkspace`).

## 7. Routes — in `routes/workspace.php`

```php
Route::resource('cars', CarController::class);
```

These mount under `/w/{workspace}/cars` when tenancy is on, or at `/cars` when off — no change
on your side.

## 8. Vue pages + nav

Add `resources/js/Pages/Cars/*.vue`. For workspace links use the `useWorkspace()` →
`workspaceUrl('/cars')` helper (or add an entry to `composables/useSidebarItems.ts`); it emits the
...
`use BelongsToWorkspace` line is the whole isolation story.
