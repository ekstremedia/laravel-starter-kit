...
**Notifications** — DB-backed + MJML email. Add one: `make:notification`, use `UsesEmailTemplate`, `via()` → `['database','mail']`, `toArray()` → `['title','message','icon']`, seed template rows for both locales in `EmailTemplateSeeder`. Editable at `/admin/mail`.
